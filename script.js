...
function collectFormData() {
    const formData = {
        basic: {
            ...
            careerSummary: getRichEditorContent(document.querySelector('#careerSummary')),
    ...
    projectItems.forEach(item => {
        ...
        richEditors.forEach(editor => {
            const name = editor.getAttribute('name');
            if (name) {
                project[name] = getRichEditorContent(editor);
            }
        });
...
function collectSkillsData() {
    ...
    const allSkills = [frontendSkills, backendSkills, databaseSkills, aiSkills, otherSkills]
...
// 기술스택 데이터 복원 함수
function restoreSkillsData(skillsString) {
    ...
    console.log('Database:', databaseSkills);
    console.log('AI:', aiSkills);
    console.log('Other:', otherSkills);
...
function restoreFormData(formData) {
    ...
    if (formData.basic) {
        ...
        // careerSummary는 Rich Text Editor로 복원
        if (formData.basic.careerSummary) {
            console.log('📝 careerSummary Rich Text Editor 복원 중...');
            const careerSummaryElement = document.querySelector('#careerSummary');
            if (careerSummaryElement) {
                setRichEditorContent(careerSummaryElement, formData.basic.careerSummary);
                console.log('✅ careerSummary 복원 완료');
            } else {
                console.log('❌ careerSummary Rich Text Editor를 찾을 수 없음');
            }
        }
    ...
    restoreDynamicItems('projectContainer', 'project-item', formData.projects || []);
}
...
function restoreItemData(itemElement, itemData) {
    ...
    richEditors.forEach((editor, index) => {
        ...
        // portfolioDescription과 projectDescription도 처리
        ...
        else if (itemData.projectDescription !== undefined) {
            setRichEditorContent(editor, itemData.projectDescription || '');
            console.log('✅ projectDescription 복원 완료');
        }
        // trainingContent도 처리
    ...
    portfolioItem.innerHTML = `
        ...
        <div class="form-group">
            ...
            <div class="rich-editor-container">
                <div class="rich-editor" name="portfolioDescription" contenteditable="true" data-placeholder="포트폴리오에 대한 간단한 설명을 입력하세요"></div>
            </div>
        </div>
...
function addProject() {
    ...
    projectItem.innerHTML = `
        <div class="form-row">
            <div class="form-group">
                <label>프로젝트명</label>
                <input type="text" name="projectName" placeholder="예: 쇼핑몰 웹사이트 개발">
            </div>
            <div class="form-group">
                <label>역할</label>
                <input type="text" name="projectRole" placeholder="예: 팀장, 개발자, 디자이너">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>시작일</label>
                <input type="text" name="projectStartDate" placeholder="예: 2023.01">
            </div>
            <div class="form-group">
                <label>완료일</label>
                <input type="text" name="projectEndDate" placeholder="예: 2023.06">
            </div>
            <div class="form-group">
                <label>진행상태</label>
        ...
        <div class="form-group">
            <label>프로젝트 설명</label>
            <div class="rich-editor-container">
                <div class="rich-editor" name="projectDescription" contenteditable="true" data-placeholder="프로젝트의 목적, 사용 기술, 주요 성과 등을 입력하세요"></div>
            </div>
        </div>
        <button type="button" class="remove-btn" onclick="removeItem(this)">삭제</button>
    `;
    ...
    // 새로 추가된 Rich Text Editor 초기화
    const newEditor = projectItem.querySelector('.rich-editor');
    if (newEditor) {
        // 툴바 생성 및 이벤트 리스너 설정
        createToolbarForEditor(newEditor, 'project');
        setupEditorEventListeners(newEditor, 'project');
        setupPlaceholder(newEditor);
    }
}
...
function extractResumeHighlights() {
    try {
        ...
        if (formData.projects && formData.projects.length > 0) {
            ...
            highlights.push(`📁 프로젝트 ${projectCount}건 수행`);
        }
...
function generateResumeHTML(data) {
    const skillsList = data.skills ? data.skills.split(',').map(skill => skill.trim()).filter(skill => skill) : [];
                                ...
                                <div class="skill-tags">
                                    ${skillsList.filter(skill => {
                                        ...
                                        return skillParts.some(part => 
                                            ['html', 'css', 'javascript', 'react', 'vue', 'angular', 'jquery', 'bootstrap', 'sass', 'less', 'typescript', 'flutter', 'next.js', 'nuxt.js', 'gatsby', 'webpack', 'vite', 'tailwind', 'styled-components', 'material-ui', 'antd', 'chakra-ui'].includes(part)
                                        );
                            ...
                            <div class="skill-category">
                                ...
                                <div class="skill-tags">
                                    ${skillsList.filter(skill => {
                                        ...
                                        const skillParts = skillLower.replace(/[()]/g, ' ').split(/\s+/).filter(part => part.length > 0);
                                        return skillParts.some(part => 
                                            ['node.js', 'nodejs', 'python', 'java', 'php', 'c#', 'c++', 'go', 'ruby', 'django', 'express', 'spring', 'springboot', 'fastapi', 'flask', 'laravel', 'asp.net', 'dotnet', 'kotlin', 'scala', 'rust', 'elixir', 'nestjs', 'koa', 'hapi', 'adonis', 'strapi', 'graphql', 'rest', 'api', 'microservices', 'serverless', 'lambda', 'azure functions', 'google cloud functions'].includes(part)
                                        );
                                    }).map(skill => `<span class="skill-tag">${skill}</span>`).join('')}
                                </div>
                            </div>
                            <div class="skill-category">
                                ...
                                <div class="skill-tags">
                                    ${skillsList.filter(skill => {
                                        ...
                                        const skillParts = skillLower.replace(/[()]/g, ' ').split(/\s+/).filter(part => part.length > 0);
                                        return skillParts.some(part => 
                                            ['mysql', 'postgresql', 'mongodb', 'redis', 'sqlite', 'oracle', 'sql server', 'firebase', 'firestore', 'dynamodb', 'cassandra', 'neo4j', 'elasticsearch', 'influxdb', 'couchdb', 'mariadb', 'cockroachdb', 'timescaledb', 'supabase', 'planetscale'].includes(part)
                                        );
                                    }).map(skill => `<span class="skill-tag">${skill}</span>`).join('')}
                                </div>
                            </div>
                            <div class="skill-category">
                                ...
                                <div class="skill-tags">
                                    ${skillsList.filter(skill => {
                                        ...
                                        const skillParts = skillLower.replace(/[()]/g, ' ').split(/\s+/).filter(part => part.length > 0);
                                        return skillParts.some(part => 
                                            ['openai', 'gpt', 'gemini', 'tensorflow', 'pytorch', 'scikit-learn', 'sklearn', 'keras', 'pandas', 'numpy', 'matplotlib', 'seaborn', 'plotly', 'jupyter', 'colab', 'huggingface', 'transformers', 'bert', 'gpt-3', 'gpt-4', 'claude', 'langchain', 'llama', 'stable diffusion', 'midjourney', 'dalle', 'chatgpt', 'bard', 'copilot', 'github copilot', 'mlflow', 'kubeflow', 'airflow', 'spark', 'hadoop', 'kafka', 'flink'].includes(part)
                                        );
                                    }).map(skill => `<span class="skill-tag">${skill}</span>`).join('')}
                                </div>
                            </div>
                            <div class="skill-category">
                                ...
                                <div class="skill-tags">
                                    ${skillsList.filter(skill => {
                                        ...
                                        const skillParts = skillLower.replace(/[()]/g, ' ').split(/\s+/).filter(part => part.length > 0);
                                        return !skillParts.some(part => 
                                            ['html', 'css', 'javascript', 'react', 'vue', 'angular', 'jquery', 'bootstrap', 'sass', 'less', 'typescript', 'flutter', 'next.js', 'nuxt.js', 'gatsby', 'webpack', 'vite', 'tailwind', 'styled-components', 'material-ui', 'antd', 'chakra-ui', 'node.js', 'nodejs', 'python', 'java', 'php', 'c#', 'c++', 'go', 'ruby', 'django', 'express', 'spring', 'springboot', 'fastapi', 'flask', 'laravel', 'asp.net', 'dotnet', 'kotlin', 'scala', 'rust', 'elixir', 'nestjs', 'koa', 'hapi', 'adonis', 'strapi', 'graphql', 'rest', 'api', 'microservices', 'serverless', 'lambda', 'azure functions', 'google cloud functions', 'mysql', 'postgresql', 'mongodb', 'redis', 'sqlite', 'oracle', 'sql server', 'firebase', 'firestore', 'dynamodb', 'cassandra', 'neo4j', 'elasticsearch', 'influxdb', 'couchdb', 'mariadb', 'cockroachdb', 'timescaledb', 'supabase', 'planetscale', 'openai', 'gpt', 'gemini', 'tensorflow', 'pytorch', 'scikit-learn', 'sklearn', 'keras', 'pandas', 'numpy', 'matplotlib', 'seaborn', 'plotly', 'jupyter', 'colab', 'huggingface', 'transformers', 'bert', 'gpt-3', 'gpt-4', 'claude', 'langchain', 'llama', 'stable diffusion', 'midjourney', 'dalle', 'chatgpt', 'bard', 'copilot', 'github copilot', 'mlflow', 'kubeflow', 'airflow', 'spark', 'hadoop', 'kafka', 'flink'].includes(part)
                                        );
                                    }).map(skill => `<span class="skill-tag">${skill}</span>`).join('')}
                                </div>
                            </div>
            ...
            <div class="resume-body">
                ...
                <div class="resume-section">
                    ...
                    ${data.portfolio.map(portfolio => `
                        <div class="portfolio-item-resume">
                            <div class="item-header">
                                <div class="content">
                ...
                <div class="resume-section">
                    ...
                    ${data.projects.map(project => `
                        <div class="project-item-resume">
                            <div class="item-header">
                                <div class="year">${formatYearRange(project.projectStartDate, project.projectEndDate)}</div>
                                <div class="content">
                                    <h4>${project.projectName}</h4>
                                    <div class="period">${project.projectRole || ''} | ${project.projectStatus || ''}</div>
                                    ${project.projectDescription ? `<div class="project-description">${project.projectDescription}</div>` : ''}
                                </div>
                            </div>
                        </div>
                    `).join('')}
...
function getProjectStatusSummary(projects) {
    if (!projects || projects.length === 0) return '없음';
    
    const statusCounts = {};
    projects.forEach(project => {
        const status = project.projectStatus || '진행중';
